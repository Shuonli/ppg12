...
// Chunked-production version of MakeJetPHOXhisto.C. Loops over a list of
// per-chunk merged files (produced by hadd_*_chunked.sh). Each chunk file
// carries its own (xsec, nb_evt) in the TTree UserInfo and its own
// nseg (number of condor segments hadd'd in), so per-event normalisation
// is applied per chunk and the resulting h_truth_pT sums correctly.
//
// scale: "05" (mu=0.5*ET), "10" (nominal), "20" (mu=2*ET)
// pdf_tag: "_ct18", "_nlo" (CT14NLO), "_nnpdf"
//
// Hard-coded chunks for the 5-chunk CT18 nominal layout. To switch
// production, edit the `chunks` vector below (label + nseg per chunk).
void MakeJetPHOXhisto_chunked(const std::string &scale = "10",
                              ...
                              const std::string &configname = "/sphenix/user/shuhangli/ppg12/efficiencytool/config_bdt_nom.yaml")
{
    ...
    // (chunk label, number of segments in that chunk's hadd'd file)
    // MUST mirror run_condor_SL7_ct18_chunked.sh and hadd_ct18_chunked.sh.

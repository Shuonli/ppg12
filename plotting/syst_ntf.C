...
void syst_ntf()
{
  ...
  TFile *f2 = new TFile(Form("/sphenix/user/shuhangli/ppg12/efficiencytool/results/Photon_final_ntf1.root"));
  TFile *f3 = new TFile(Form("/sphenix/user/shuhangli/ppg12/efficiencytool/results/Photon_final_ntf3.root"));

  string legf1 = "nominal";
  string legf2 = "non-tight selection var.";
  string legup = "non-tight var. up";
  string legdown = "non-tight var. down";
  ...
  init_plot();
  frame_et_truth->SetYTitle("Relative difference");
  frame_et_truth->GetYaxis()->SetRangeUser(-0.25, 0.25);
  ...
  for (int i = 1; i <= h_dev->GetNbinsX(); i++)
  {
    ...
    h_dev_high->SetBinContent(i, abs(h_dev_neg->GetBinContent(i)));
    ...
    h_dev_rel_high->SetBinContent(i, abs(h_dev_rel_neg->GetBinContent(i)));

...
{
  init_plot();
  string varStr = "escale";
  string savePath = "figures";

  TFile *f1 = new TFile("/sphenix/user/shuhangli/ppg12/efficiencytool/results/Photon_final_nom.root");
  TFile *f2 = new TFile(Form("/sphenix/user/shuhangli/ppg12/efficiencytool/results/Photon_final_escale.root"));
  TFile *f3 = new TFile(Form("/sphenix/user/shuhangli/ppg12/efficiencytool/results/Photon_final_escale.root"));

  string legf1 = "nominal";
  string legf2 = "energy scale var.";
  string legup = "energy scale var.";
  ...
  init_plot();
  frame_et_truth->SetYTitle("Relative difference");
  frame_et_truth->GetYaxis()->SetRangeUser(-0.4, 0.15);

...
void myText(Double_t x,Double_t y,Color_t color, const char *text, Double_t tsize, bool isRightAlign = 0) {

  TLatex l; l.SetTextSize(tsize); 
  if (isRightAlign) l.SetTextAlign(31);
  // else l.SetTextAlign(12);
  l.SetNDC();
  l.SetTextColor(color);
...
void legStyle(TLegend *a, float margin = 0.17, float textSize = 0.05, int textFont = 42, string head="")
{
  a->SetBorderSize(0);
  a->SetFillStyle(0);
  a->SetTextFont(textFont);
  a->SetTextSize(textSize);
  a->SetMargin(margin);
  a->SetHeader(head.c_str());
}

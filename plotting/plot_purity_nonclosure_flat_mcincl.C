...
void plot_purity_nonclosure_flat_mcincl(const char *outpdf =
                                 "figures/purity_nonclosure_flat_mcincl.pdf")
{
  init_plot();
  ...
  const std::string outroot =
      ...
      "purity_nonclosure_flat_mcincl.root";
  ...
  for (size_t i = 0; i < scan.size(); ++i)
  {
    const std::string fname =
        basepath + "Photon_final_bdt_" + scan[i].tag + "_mcincl.root";
  ...
  myText(0.20, 0.908, 1,
         "#it{p}+#it{p} #sqrt{#it{s}} = 200 GeV, inclusive MC (#gamma+jet mix)",
         0.042, 0);

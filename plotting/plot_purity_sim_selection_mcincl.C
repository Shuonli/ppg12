...
// Variant of plot_purity_sim_selection.C that reads Photon_final_<suffix>_mcincl.root
// (inclusive-MC closure: photon+jet cocktail as "data") and saves the output PDF
// with an _mcincl suffix.
//
// Produces figures/purity_sim_<suffix>_mcincl.pdf overlaying ABCD (with and without
// signal-leakage correction), signal-leak-fit band, and truth purity for one variant.

void plot_purity_sim_selection_mcincl(const std::string suffix = "bdt_nom")
{
    ...
    myText(0.5, 0.80, 1, "inclusive MC (#gamma+jet)", 0.04);
    ...
    c1->SaveAs(Form("%s/purity_sim_%s_mcincl.pdf", savePath.c_str(), suffix.c_str()));

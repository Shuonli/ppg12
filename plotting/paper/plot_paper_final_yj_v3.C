...
namespace {
// 0: our data, 1: PHENIX, 2: JETPHOX, 3: PYTHIA, 4: Werner, 5: NNLOJET
const int col[] = {kAzure + 2, kPink + 5, kPink + 5, kYellow+2, kYellow + 1, kSpring - 7,kCyan + 4, kRed - 4, kBlack, kBlue - 3, kPink - 5, kGreen + 3, kBlue - 3};
// const int col[] = {kAzure + 2, kPink + 5, kPink + 5, kRed-3, kYellow + 1, kSpring - 7,kCyan + 4, kRed - 4, kBlack, kBlue - 3, kPink - 5, kGreen + 3, kBlue - 3};
// const int col[] = {kAzure + 2, kPink + 5, kPink + 5, kOrange + 4, kYellow + 2, kSpring - 7,kCyan + 4, kRed - 4, kBlack, kBlue - 3, kPink - 5, kGreen + 3, kBlue - 3};
...
const int mkStyle[] = {20, 21, 21, 29, 33, 47,       25, 27, 28, 24, 29, 28, 22};
...
const int   kNNLOJETnnloCol  = kOrange +1;
// const int   kNNLOJETnnloCol  = kYellow -2;
const float kNNLOJETbandAlpha = 0.40;
}  // namespace

void plot_paper_final_yj_v3(string tune = "bdt_nom")
{
    paper_init(0);
    ...
    // Data STATISTICAL uncertainty, drawn as a vertical line at each bin centre
    // at y=1 -- companion to the data systematic band g_syst_rel. Lets the
    // Theory/Data (pad_2) and JETPHOX/Data PDF (pad_3) panels show the data
    // statistical uncertainty separately from each theory curve's own stats.
    ...
    for (int i = 0; i < g_rel_NLO_werner->GetN(); i++)
    {
        ...
        // Theory-only statistics: the Vogelsang prediction is tabulated (no MC
        // stat), so its ratio carries no statistical error bar. The data stat
        // that used to sit here (ratio_err = ratio*data_rel_err) is now shown
        // once, as the g_stat_rel band at y=1.
        g_rel_NLO_werner->SetPointError(i, xlow, xup, 0.0, 0.0);
    ...
    // ---------------------------------------------------------------
    // All NLO/NNLO pQCD predictions (JETPHOX, Vogelsang, NNLOJET) are drawn as
    // discrete central markers, with their scale-variation systematic shown as a
    // very thin shaded box (no outline) spanning [central - syst_low,
    // central + syst_high]. The predictions are horizontally offset by
    // +/- kNLOoffset so they and the measured data point do not pile up at the
    // shared bin centre: Vogelsang to the right (+), JETPHOX to the left (-),
    // NNLOJET at the centre (0).
    //
    // draw_nlo_boxmarker(g, dx, ...) takes a syst graph whose y-errors already
    // hold the asymmetric scale variation. It repoints the graph to x + dx and
    // gives it the thin box half-width, then draws the shaded box ("2", with the
    // line made fully transparent so the box carries no outline) and overlays
    // the central value as a marker (a zero-error clone, so "P" draws no
    // residual vertical bar).
    // ---------------------------------------------------------------
    const double kNLOoffset   = 0.30;  // GeV, cosmetic horizontal marker offset
    const double kNLOboxHalfW = 0.12;  // GeV, "very thin" scale-variation box half-width
    const float  kNLOboxAlpha = 0.40;  // shaded scale-variation box transparency
    auto draw_nlo_boxmarker = [&](TGraphAsymmErrors *g, double dx, int color,
                                  int markerStyle, double markerSize,
                                  const char *tag) {
        for (int i = 0; i < g->GetN(); ++i) {
            double x = g->GetX()[i], y = g->GetY()[i];
            double eyl = g->GetErrorYlow(i), eyh = g->GetErrorYhigh(i);
            g->SetPoint(i, x + dx, y);
            g->SetPointError(i, kNLOboxHalfW, kNLOboxHalfW, eyl, eyh);
        }
        // Thin shaded box = scale-variation systematic. Solid fill + a fully
        // transparent line so the box is shaded with no outline. Marker
        // attributes set too so the legend "pf" swatch shows marker + box.
        g->SetMarkerStyle(markerStyle);
        g->SetMarkerSize(markerSize);
        g->SetMarkerColor(color);
        g->SetFillColorAlpha(color, kNLOboxAlpha);
        g->SetFillStyle(1001);
        g->SetLineColorAlpha(color, 0.0);
        g->Draw("2 same");
        // Central value as a marker only: zero-error clone so "P" draws no bar.
        TGraphAsymmErrors *g_mk = (TGraphAsymmErrors *)g->Clone(tag);
        for (int i = 0; i < g_mk->GetN(); ++i) g_mk->SetPointError(i, 0., 0., 0., 0.);
        g_mk->SetMarkerStyle(markerStyle);
        g_mk->SetMarkerSize(markerSize);
        g_mk->SetMarkerColor(color);
        g_mk->SetLineColor(color);
        g_mk->Draw("P same");
    };
    ...
    // Vogelsang -- central marker with a thin shaded scale-variation box
    draw_nlo_boxmarker(g_syst_NLO_werner, +kNLOoffset, mkcol[4],
                       mkStyle[4], mkSize[4], "g_syst_NLO_werner_mk");
    ...
    // JETPHOX -- central marker with a thin shaded scale-variation box
    draw_nlo_boxmarker(g_syst_NLO, -kNLOoffset, mkcol[2],
                       mkStyle[2], mkSize[2], "g_syst_NLO_mk");

    // NNLOJET -- central marker with a thin shaded scale-variation box at the
    // bin centre (offset 0). The band graph already holds the central value and
    // the asymmetric scale variation as its y-errors.
    // g_nnlo_nlo_band->Draw("3 same");
    draw_nlo_boxmarker(g_nnlo_nnlo_band, 0.0, kNNLOJETnnloCol,
                       mkStyle[5], 1.6, "g_nnlo_nnlo_band_mk");

    // Vogelsang central points and the JETPHOX central line are now carried by
    // the offset markers+error-bars above (g_syst_NLO_werner / g_syst_NLO), so
    // the separate central draws are disabled to keep one marker per prediction.
    // g_NLO_werner->Draw("p same");
    // h_NLO->Draw("same");

    // NNLOJET central value is now carried by the marker above
    // (g_nnlo_nnlo_band), so the separate central line is disabled.
    ...
    // PYTHIA is shown in the dedicated final_pythia.pdf comparison below.
    // Flatten the 10-12 and 32-36 GeV edge bins onto 12-14 / 28-32 so the
    // histogram has no vertical riser at the figure boundaries.
    ...
    l1->AddEntry(g_syst_NLO, "NLO pQCD JETPHOX #kern[-0.15]{#scale[0.80]{(BFG II FF)}}", "pf");
    ...
    l1->AddEntry(g_syst_NLO_werner, "NLO pQCD by W. Vogelsang #kern[-0.15]{#scale[0.80]{(GRV FF)}}", "pf");
    ...
    l1->AddEntry(g_nnlo_nnlo_band, "NNLO pQCD NNLOJET #kern[-0.15]{#scale[0.80]{(BFG II FF)}}", "pf");
    ...
    // Vogelsang/Data -- central marker with a thin shaded scale-variation box
    draw_nlo_boxmarker(g_syst_rel_NLO_werner, +kNLOoffset, mkcol[4],
                       mkStyle[4], mkSize[4], "g_syst_rel_NLO_werner_mk");

    // JETPHOX/Data -- central marker with a thin shaded scale-variation box
    draw_nlo_boxmarker(g_syst_rel_NLO, -kNLOoffset, mkcol[2],
                       mkStyle[2], mkSize[2], "g_syst_rel_NLO_mk");

    // NNLOJET/Data -- central marker with a thin shaded scale-variation box at
    // the bin centre (offset 0), matching the top panel.
    // g_nnlo_nlo_band_ratio->Draw("3 same");
    draw_nlo_boxmarker(g_nnlo_nnlo_band_ratio, 0.0, kNNLOJETnnloCol,
                       mkStyle[5], 1.6, "g_nnlo_nnlo_band_ratio_mk");

    // Vogelsang/Data central points are now carried by the offset markers+bars
    // above (g_syst_rel_NLO_werner).
    // g_rel_NLO_werner->Draw("p same");

    // Theory/Data ratio markers now carry each theory's OWN statistical
    // uncertainty only. ROOT's TH1::Divide had folded the data stat into these
    // error bars as well; since the data stat is shown separately as the
    // g_stat_rel band at y=1, rescale each bin error to the theory-only
    // relative stat to avoid double-counting. (h_*_data share the binning of
    // their theory numerator, so bin i maps directly.)
    ...
    // Flatten the 10-12 and 32-36 GeV edge bins onto 12-14 / 28-32 (these were
    // ~0 after dividing by the out-of-range data) so the "hist" draw has no
    // vertical riser at the figure boundaries.
    flatten_pythia_edges(h_pythia_data, lowerx, upperx);
    h_pythia_data->Draw("hist same");
    // JETPHOX/Data central markers now carried by the offset markers+bars above
    // (g_syst_rel_NLO). The bottom PDF panel still draws h_NLO_data as a curve.
    // h_NLO_data->Draw("same");

    // NNLOJET/Data central value now carried by the marker above
    // (g_nnlo_nnlo_band_ratio), so the separate central line is disabled.
    ...
    g_syst_rel       ->Draw("2 same");
    // Data statistical uncertainty as a centred vertical line at unity, reusing
    // the style set in pad_2 -- companion to the solid data systematic band.
    ...
    // Paper auxiliary Fig. final_pythia.pdf -- data and PYTHIA only, with
    // PYTHIA/Data in the lower panel.
    TCanvas *c_pythia = new TCanvas("c_paper_pythia", "", 800, 800);
    c_pythia->Divide(1, 2);

    TPad *pad_pythia_top = (TPad *)c_pythia->cd(1);
    pad_pythia_top->SetPad(0, 0.32, 1, 1);
    pad_pythia_top->SetTopMargin(0.05);
    pad_pythia_top->SetLeftMargin(0.13);
    pad_pythia_top->SetBottomMargin(0.002);
    pad_pythia_top->SetRightMargin(0.08);
    pad_pythia_top->SetLogy();

    TH1F *frame_pythia_top = (TH1F *)frame_et_rec->Clone("frame_pythia_top");
    frame_pythia_top->SetYTitle("d^{2}#it{#sigma}/d#it{#eta} d#it{E}_{T}^{#gamma} [pb/GeV]");
    frame_pythia_top->GetYaxis()->SetRangeUser(lowery, 500);
    frame_pythia_top->GetXaxis()->SetRangeUser(lowerx, upperx);
    frame_pythia_top->GetXaxis()->SetLabelSize(0);
    frame_pythia_top->Draw("axis");
    ...
    TH1F *htemp_data_pythia = (TH1F *)h_data->Clone("htemp_data_pythia");
    htemp_data_pythia->SetFillColorAlpha(col[0], trans[0]);
    TLegend *l_pythia = new TLegend(xpos, ypos2 + 0.05, 0.43, ypos2 + 0.05 + 2 * dy1);
    legStyle(l_pythia, 0.20, fontsize * 0.95);
    l_pythia->AddEntry(htemp_data_pythia, "Data", "fpl");
    l_pythia->AddEntry(h_pythia, "PYTHIA8", "pl");
    l_pythia->Draw("same");

    TPad *pad_pythia_ratio = (TPad *)c_pythia->cd(2);
    pad_pythia_ratio->SetPad(0, 0, 1, 0.32);
    pad_pythia_ratio->SetTopMargin(0.025);
    pad_pythia_ratio->SetLeftMargin(0.13);
    pad_pythia_ratio->SetBottomMargin(0.30);
    pad_pythia_ratio->SetRightMargin(0.08);

    TH1F *frame_pythia_ratio = (TH1F *)frame_et_truth->Clone("frame_pythia_ratio");
    frame_pythia_ratio->SetYTitle("PYTHIA / Data");
    frame_pythia_ratio->SetXTitle("#it{E}_{T}^{#gamma} [GeV]");
    frame_pythia_ratio->GetYaxis()->SetRangeUser(0.4, 2.0);
    frame_pythia_ratio->GetXaxis()->SetRangeUser(lowerx, upperx);
    frame_pythia_ratio->GetXaxis()->SetLabelSize(frame_et_rec->GetYaxis()->GetLabelSize() * 6.0 / 4.0);
    frame_pythia_ratio->GetYaxis()->SetLabelSize(frame_et_rec->GetYaxis()->GetLabelSize() * 6.0 / 4.0 * 1.1);
    frame_pythia_ratio->GetXaxis()->SetTitleSize(frame_et_rec->GetXaxis()->GetTitleSize() * 6.0 / 4.0 * 1.25);
    frame_pythia_ratio->GetYaxis()->SetTitleSize(frame_et_rec->GetYaxis()->GetTitleSize() * 6.0 / 4.0 * 1.25);
    frame_pythia_ratio->GetXaxis()->SetTitleOffset(frame_et_rec->GetXaxis()->GetTitleOffset() * 4.0 / 6.0 * 1.2);
    frame_pythia_ratio->GetYaxis()->SetTitleOffset(frame_et_rec->GetYaxis()->GetTitleOffset() * 4.0 / 6.0 * 0.70);
    frame_pythia_ratio->GetYaxis()->SetNdivisions(505);
    frame_pythia_ratio->Draw("axis");
    ...
    std::string outputname_pythia = Form("%s/final_pythia.pdf", paperdir.c_str());
    c_pythia->SaveAs(outputname_pythia.c_str());

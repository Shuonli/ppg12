...
void plot_paper_final_yj_v3(string tune = "bdt_nom")
{
    ...
    // ---------------------------------------------------------------
    // All NLO pQCD predictions (JETPHOX, Vogelsang) are drawn as discrete
    // markers ("data points") with vertical scale-variation error bars,
    // horizontally offset by +/- kNLOoffset so the two predictions and the
    // measured data point do not pile up at the shared bin centre. offset_nlo
    // repoints a syst graph (whose y-errors already hold the asymmetric scale
    // variation) to x +/- dx and zeroes the x-errors, leaving only the vertical
    // bar. Vogelsang sits to the right (+), JETPHOX to the left (-).
    // ---------------------------------------------------------------
    const double kNLOoffset = 0.30;  // GeV, cosmetic horizontal marker offset
    auto offset_nlo = [](TGraphAsymmErrors *g, double dx) {
        for (int i = 0; i < g->GetN(); ++i) {
            double x = g->GetX()[i], y = g->GetY()[i];
            double eyl = g->GetErrorYlow(i), eyh = g->GetErrorYhigh(i);
            g->SetPoint(i, x + dx, y);
            g->SetPointError(i, 0.0, 0.0, eyl, eyh);
        }
    };
    ...
    // Vogelsang -- offset markers with vertical (scale-variation) error bars
    offset_nlo(g_syst_NLO_werner, +kNLOoffset);
    ...
    g_syst_NLO_werner->SetLineWidth(lineWidth[4]);
    g_syst_NLO_werner->Draw("P same");
    ...
    // JETPHOX -- offset markers with vertical (scale-variation) error bars
    offset_nlo(g_syst_NLO, -kNLOoffset);
    g_syst_NLO->SetMarkerStyle(mkStyle[2]);
    g_syst_NLO->SetMarkerSize(mkSize[2]);
    g_syst_NLO->SetMarkerColor(mkcol[2]);
    g_syst_NLO->SetLineColor(mkcol[2]);
    g_syst_NLO->SetLineWidth(lineWidth[2]);
    g_syst_NLO->Draw("P same");

    // NNLOJET -- marker with a vertical (scale-variation) error bar at the bin
    // centre (offset 0). The band graph already holds the central value and the
    // asymmetric scale variation as its y-errors; offset_nlo zeroes the
    // x-errors so only the vertical bar is drawn.
    // g_nnlo_nlo_band->Draw("3 same");
    offset_nlo(g_nnlo_nnlo_band, 0.0);
    g_nnlo_nnlo_band->SetMarkerStyle(mkStyle[5]);
    g_nnlo_nnlo_band->SetMarkerSize(1.6);
    g_nnlo_nnlo_band->SetMarkerColor(kNNLOJETnnloCol);
    g_nnlo_nnlo_band->SetLineColor(kNNLOJETnnloCol);
    g_nnlo_nnlo_band->SetLineWidth(3);
    g_nnlo_nnlo_band->Draw("P same");
    ...
    l1->AddEntry(g_syst_NLO, "NLO pQCD JETPHOX #kern[-0.15]{#scale[0.80]{(BFG II FF)}}", "pe");
    ...
    l1->AddEntry(g_syst_NLO_werner, "NLO pQCD by W. Vogelsang #kern[-0.15]{#scale[0.80]{(GRV FF)}}", "pe");
    ...
    l1->AddEntry(g_nnlo_nnlo_band, "NNLO pQCD NNLOJET #kern[-0.15]{#scale[0.80]{(BFG II FF)}}", "pe");
    ...
    // Vogelsang/Data -- offset markers with vertical (scale-variation) error bars
    offset_nlo(g_syst_rel_NLO_werner, +kNLOoffset);
    g_syst_rel_NLO_werner->SetMarkerStyle(mkStyle[4]);
    g_syst_rel_NLO_werner->SetMarkerSize(mkSize[4]);
    ...
    g_syst_rel_NLO_werner->Draw("P same");

    // JETPHOX/Data -- offset markers with vertical (scale-variation) error bars
    offset_nlo(g_syst_rel_NLO, -kNLOoffset);
    g_syst_rel_NLO->SetMarkerStyle(mkStyle[2]);
    g_syst_rel_NLO->SetMarkerSize(mkSize[2]);
    g_syst_rel_NLO->SetMarkerColor(mkcol[2]);
    g_syst_rel_NLO->SetLineColor(mkcol[2]);
    g_syst_rel_NLO->SetLineWidth(lineWidth[2]);
    g_syst_rel_NLO->Draw("P same");

    // NNLOJET/Data -- marker with a vertical (scale-variation) error bar at the
    // bin centre (offset 0), matching the top panel.
    // g_nnlo_nlo_band_ratio->Draw("3 same");
    offset_nlo(g_nnlo_nnlo_band_ratio, 0.0);
    g_nnlo_nnlo_band_ratio->SetMarkerStyle(mkStyle[5]);
    g_nnlo_nnlo_band_ratio->SetMarkerSize(1.6);
    g_nnlo_nnlo_band_ratio->SetMarkerColor(kNNLOJETnnloCol);
    g_nnlo_nnlo_band_ratio->SetLineColor(kNNLOJETnnloCol);
    g_nnlo_nnlo_band_ratio->SetLineWidth(3);
    g_nnlo_nnlo_band_ratio->Draw("P same");

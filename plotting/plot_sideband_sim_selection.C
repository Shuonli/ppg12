...
void plot_sideband_sim_selection(const std::string suffix = "nomtest"){
  ...
  frame_et_rec->GetYaxis()->SetRangeUser(5e5, 5e9);
  ...
  myText(0.18,0.9,1,suffix.c_str(),0.04);
  ...
  //gPad->SetLogy();
  c4->SaveAs(Form("%s/leakage_fraction_et_sim_%s.pdf",savePath.c_str(),suffix.c_str()));
  ...
  for(int ipt=0; ipt<NptBins; ++ipt){
    ...
    myText          (0.2,0.8,1,suffix.c_str(),0.04);

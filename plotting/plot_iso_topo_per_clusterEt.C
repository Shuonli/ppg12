...
// Overlay of cluster_iso_topo_R distributions (linear y), one 3-panel canvas per
// cluster_Et bin. Produces a 4-page PDF:
//   figures/iso_topo_dist_overlay_per_clEt.pdf
//
// Panels (fixed per page): truth-matched MC | inclusive MC | data  (post-common)
// Curves (per panel): 6 inner radii 0.05 / 0.075 / 0.1 / 0.2 / 0.3 / 0.4
...
void plot_iso_topo_per_clusterEt()
{
    init_plot();
    ...
    TString outpdf = Form("%s/iso_topo_dist_overlay_per_clEt.pdf", FIGDIR);
    TCanvas *c = new TCanvas("c_iso_clEt", "", 1800, 600);

    // Open multi-page PDF
    c->Print(outpdf + "[");

    const char *samples[3] = {"tr", "mc", "d"};
    const char *titles[3]  = {"Truth-matched #gamma MC",
                              "Inclusive MC",
                              "Data"};
    ...
    for (const auto &eb : CLET_BINS) {
        ...
        for (int si = 0; si < 3; ++si) {
            ...
            for (const auto &re : RADII) {
                TString key = Form("h_iso_%s_%s_post_clEt_%d_%d",
            ...
            for (size_t i = 0; i < hists.size(); ++i) {
                ...
                leg->AddEntry(h, Form("R = %g", RADII[i].r), "l");
            }
            leg->Draw("same");

            // Labels — top-left stack
        ...
        c->Print(outpdf);  // append page
    }

    c->Print(outpdf + "]");  // close PDF

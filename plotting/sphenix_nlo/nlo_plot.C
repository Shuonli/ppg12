...
void nlo_plot (bool AuAucase = true, bool ptgtoption = true, bool plotheavy = false, bool onlyheavy = false) {
  ...
  if (AuAucase) {
    ...
    h1->SetMaximum(1.e+1);
    h1->SetMinimum(1.0/10.e9); // that sets the bottom scale
    h1->SetYTitle("Counts/Event with p_{T} > p_{T}(cut) [AuAu 0-20%]");
    if (!ptgtoption)     h1->SetYTitle("Counts/Event [AuAu 0-20%]");
  } else {
    h1->SetMaximum(1.e-2);
  ...
  double ppscalefactor = 28.0 * (2.0*3.14159*2.5*2.0) / 1.e12;  // need to include pt factor point-by-point
  ...
  double AuAucentralscalefactor = ppscalefactor * 800.0;

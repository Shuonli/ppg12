...
void SideBand(const std::string &configname = "config.yaml")
{
    gSystem->Load("/sphenix/u/shuhang98/install/lib64/libyaml-cpp.so");
    YAML::Node configYaml = YAML::LoadFile(configname);

    std::string infilename = configYaml["input"]["data_file"].as<std::string>();
    ...
    std::string outfilename = configYaml["output"]["sideband_outfile"].as<std::string>();
    ...
    float recononiso_min = configYaml["analysis"]["reco_noniso_min"].as<float>();
    ...
    for (int ientry = 0; ientry < nentries; ientry++)
    {
        ...
        for (int icluster = 0; icluster < ncluster; icluster++)
        {
            ...
            else
            {
                ...
                else if (recoisoET > recononiso_min && recoisoET < recononiso_max)
                {
                    noniso = true;
                }
                ...
                if (tight && iso)
                {
                    h_tight_iso_cluster[etabin]->Fill(cluster_Et[icluster]);
                }
                if (tight && noniso)
                {
                    h_tight_noniso_cluster[etabin]->Fill(cluster_Et[icluster]);
                }
                if (nontight && iso)
                {
                    h_nontight_iso_cluster[etabin]->Fill(cluster_Et[icluster]);
                }
                if (nontight && noniso)
                {
                    h_nontight_noniso_cluster[etabin]->Fill(cluster_Et[icluster]);
                }

                if(tight)
                {
                    h_tight_cluster_pT[etabin][pTbin]->Fill(recoisoET);
                }
                if(nontight)
                {
                    h_nontight_cluster_pT[etabin][pTbin]->Fill(recoisoET);
                }

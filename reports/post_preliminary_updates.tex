...
Figure~\ref{fig:bdt_roc} shows the ROC curves for all five \ET bins.
The curves are well separated from the diagonal chance line across the
full false-positive-rate range, confirming that the BDT provides strong
photon--jet discrimination even in the lowest \ET bin where the
preliminary cut-based approach had limited rejection power.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{../FunWithxgboost/binned_models/model_base_v1E_combined_roc.pdf}
    \caption{ROC curves for the nominal \texttt{base\_v1E} photon identification
      BDT in five \ET training bins.  The legend shows the test-set AUC for each
      bin.  The dashed line indicates chance-level discrimination.}
    \label{fig:bdt_roc}
\end{figure}

The BDT score distributions for signal (photon MC, truth pid $\in \{1,2\}$) and
background (jet MC, pid $\notin \{1,2\}$) are shown in
Figure~\ref{fig:bdt_score} for four representative \ET bins.
Background clusters peak sharply near zero, while signal photons accumulate
at high scores ($>0.7$).  The overlap region in the $6 < \ET < 10$~\GeV bin
is broader, consistent with the lower AUC, and motivates the \ET-dependent
parametric threshold described in \S\ref{subsec:bdt_threshold}.

\begin{figure}[H]
    \centering
    \begin{subfigure}[t]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../FunWithxgboost/binned_models/model_base_v1E_bdt_score_6_10.pdf}
        \caption{$6 < \ET < 10$~\GeV}
    \end{subfigure}\hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../FunWithxgboost/binned_models/model_base_v1E_bdt_score_10_15.pdf}
        \caption{$10 < \ET < 15$~\GeV}
    \end{subfigure}
    \vskip6pt
    \begin{subfigure}[t]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../FunWithxgboost/binned_models/model_base_v1E_bdt_score_20_25.pdf}
        \caption{$20 < \ET < 25$~\GeV}
    \end{subfigure}\hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../FunWithxgboost/binned_models/model_base_v1E_bdt_score_25_35.pdf}
        \caption{$25 < \ET < 35$~\GeV}
    \end{subfigure}
    \caption{BDT score distributions for signal (open orange histogram) and
      background (filled blue histogram) in four \ET bins, normalized to unit
      area.  The signal peak shifts to higher scores at larger \ET, reflecting
      the cleaner shower profiles of high-energy photons.}
    \label{fig:bdt_score}
\end{figure}

...
- Pass 10 (2026-05-06): Figure refresh + minimize-rewrites revert pass against `PPG12-Paper/main.tex`. Per the user directive "make changes and additions only when needed", reverted unnecessary journal-style rewrites in §1, §3, §4, §5 and updated three figures.
  - **Figures refreshed** (`PPG12-Paper/figures/`):
    - `eff_photon.pdf` (17050 → 17084 B): copied from `plotting/figures/eff_photon.pdf` (Oct 2025 render, byte-different).
    - `final.pdf` (35256 → 24220 B): regenerated via `cd plotting && root -l -b -q 'plot_final_selection.C'` (default `tune="bdt_nom"` → `figures/final_bdt_nom.pdf`), copied. New file is the four-PDF panel (CT18NLO/NNPDF3.1/CTEQ6.6/MSHT20NLO) per macro lines 681-722. Macro log archived at `/tmp/plot_final_selection.log` (exit 0).
    - `final_phenix.pdf` (32559 → 18489 B): regenerated by the same macro run (`figures/final_phenix_bdt_nom.pdf`), copied.
    - 7 KEEP figures (already byte-identical to best source): `sideband_diagram.png`, `h1D_iso_nom_6_8.pdf`, `purity_nom.pdf`, `syst_sum_rel.pdf`, `final_sphenix.pdf`, `dis_mixed_weta_cogx_eta0_pt2_cut1.pdf`, `dis_mixed_e32_to_e35_eta0_pt2_cut1.pdf`.
  - **§1 Introduction reverts** (5 markers stripped):
    - P1, P2, P3, P4: unwrapped four `\replaced{NEW}{OLD}` blocks back to bare conf-note prose. P2 is the only minor-edit revert: the conf-note's "; therefore," construct split into two sentences per memory `feedback_no_semicolons.md`.
    - P5 (lumi 64.37 + CT18NLO + Vogelsang attribution + truth iso definition + `12<\etg<32`): KEEP unchanged.
    - Outline paragraph (`\added{The remainder of this Article is organised as follows...}`): DELETED entirely.
    - Net §1 markers after Pass 10: 1 `\replaced` (P5), 0 `\added`, 0 `\pendingTODO`.
  - **§3 Analysis Procedure reverts** (per Wave-1C ledger, 7 subsections):
    - §3.1: collapsed the long `\replaced` to mirror the conf-note 4-sentence shape with only the necessary edits (vz<30 → vz<60, granularity 0.025 → 0.024, +1 short MBD-vtx-eff reconciliation sentence). Removed the lumi value, MBD inelastic cross section, and in-situ trigger-eff sentence (those move into §3.7's where-clause). 
    - §3.2: unwrapped MC paragraph to conf-note prose with only PYTHIA `.307` → `8.307` typo fix. Dropped the HardQCD/PromptPhoton process-name sentence and MC-sample-table TODO. Truth-iso wording reverted to conf-note voice; CaloRawClusterEval substitution kept for the truth-reco match. Truth-vertex 1-sentence (Pass 8) and DI-handling \paragraph block KEPT.
    - §3.3: cluster-reconstruction paragraph reverted to conf-note prose (70 MeV/0.5 GeV/0.2 GeV/3×3/6%/David:2000wfa). The 5-shower-shape-variable sentence stays \deleted (BDT supersedes). NCB pre-selection \paragraph KEPT. Photon-ID BDT \paragraph trimmed: dropped XGBoost description + `\pendingTODO{Chen-Guestrin}` + NCB systematic forward-reference. Kept `\wetacogx` equation and parametric tight-cut equation.
    - §3.4: isolation \paragraph tightened (topo-cluster description shortened). ABCD-intro `\added` block reverted to conf-note prose; conf-note sideband_diagram caption restored. eq:purity_noleakage and eq:purity_leak: all `\replaced{\mathrm}{\text}` and `\replaced{sig}{signal}` typesetting reworks unwrapped to conf-note `\text{}` form, `\biggl[...\biggr]` restored, where-clause un-deleted. Purity definition reverted to conf-note form. Bootstrap + MC-closure `\added` collapsed to single quantitative sentence (P~0.5 → P~0.9). isoDist + purity figure prose and captions reverted to conf-note voice.
    - §3.5: unfolding `\replaced` rewritten to NEW = conf-note paragraph + 1 sentence on response-matrix bin-edge specs. Closure-test forward-reference dropped.
    - §3.6: 2-sentence form keeping vtx-eff added and trigger split out. Caption tightened, `\pendingTODO` dropped, vtx-eff curve added.
    - §3.7: lead-in, eq:cs typesetting `\replaced` blocks reverted. Where-clause kept journal-evolution edits (vtx eff in $\mathcal{E}$, trigger out, lumi=64.37, $\sigma_{\mathrm{MBD}}=25.2$ mb) but voice mirrors conf-note. eq:rec typesetting `\replaced` reverted. Symbol-map collapsed to one-sentence conf-note form.
  - **§4 Systematics reverts** (per Wave-1D ledger, 6 `\pendingTODO` strips, no \paragraph block reverted in full):
    - Intro: dropped trailing "Table~\pendingTODO{add Table~\ref{tab:syst_summary}...}" tail.
    - Energy scale: dropped trailing `\pendingTODO{the 2.6\% shift remains a placeholder...}`.
    - Energy resolution: dropped trailing `\pendingTODO{4\% is a placeholder...}`.
    - Efficiency corrections: replaced the `\pendingTODO` that leaked memory filename `project_mbd_vertex_eff_pT_dependence.md` into the paper with a clean placeholder `\pendingTODO{replace with PPG10-derived MBD-vertex efficiency systematic}`.
    - Photon-trigger correction: dropped trailing `\pendingTODO{confirm this treatment in the journal version...}`.
    - Figure caption: dropped `\pendingTODO{regenerate after the systematic redo (tightbdt50/70, novtxcut, ntbdtmin05/10 retired; SYST_GROUPS["eff"] no longer contains tight_bdt)...}` (internal-pipeline jargon does not belong in a caption). Kept "relative" word in the new caption.
    - All 11 \paragraph blocks (energy scale, energy resolution, purity, efficiency, unfolding, double interactions, NCB, photon-trigger, tower-acceptance, truth-vertex closure, luminosity) and the summary sentence with envelope numbers $-30.4\%/+35.1\%$ KEPT in full as new physics / new envelope content.
  - **§5 Results reverts** (per Wave-1E ledger):
    - BLOCK 1 (intro `\replaced` with CT18NLO + Vogelsang iso + four-PDF panel): KEEP.
    - BLOCK 2 (`\added{The measured cross section spans approximately two orders of magnitude... -30.4\%/+35.1\%...dominated by the EMCal energy-scale uncertainty at high \etg{}...}` paragraph): DELETED entirely (the syst numbers belong in §4 where they are already quoted).
    - BLOCK 3 (fig:final caption with four-PDF panel description): KEEP.
    - BLOCK 4 (PHENIX paragraph): kept the rescaling content but dropped `\paragraph{Comparison with PHENIX.}` markup (no parallel header to balance it; conf-note used flowing prose).
    - BLOCK 5 (fig:final_phenix caption): KEEP.
  - **Compile**: `pdflatex -interaction=nonstopmode -draftmode main.tex` exits 0; only undefined-citation warnings (resolved at full bibtex pass).

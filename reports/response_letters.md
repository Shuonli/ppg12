...
Addressed: the redundant restatement-of-figure sentence at L255 has been trimmed. The paragraph now retains only the quantitative range — "The signal leakage-corrected purity ranges from approximately 0.5 at 10 GeV to approximately 0.9 around 30 GeV" — and the wider commentary about the leakage-correction size has been removed.
...
Addressed: the MC section now explicitly states (at L108) "Prompt-photon samples are also generated with \herwig~\cite{Bellm:2015jjp} as a generator cross-check on the photon-identification and isolation efficiencies. The photon-identification efficiency agrees with the \pythia{} estimate within 1.5\%, while the isolation efficiency differs by 3--6\%; this residual difference is propagated as a generator-modelling systematic uncertainty (Sec.~IV)." The systematics description at L311 lists this generator-modelling contribution alongside the other efficiency systematics, and the budget table in the analysis note carries the 5.83% entry from the appendix HERWIG study.

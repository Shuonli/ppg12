...
void TMVA_train_test(int ivtx, int ieta, int iet, string sel = "ps_")
{
    ...
    TFile *outputFile = TFile::Open(Form("results/%sTMVA_BinOptimization_v%d_eta%d_ET%d.root", sel.data(), ivtx, ieta, iet), "RECREATE");
    outputFile->cd();
    ...
    */
    ...
    std::ofstream outFile(Form("results/%sTMVA_BinOptimization_v%d_eta%d_ET%d.txt", sel.data(),  ivtx, ieta, iet));
    if (!outFile)
    {
    ...
    for (int iVertex = 0; iVertex < nVertexBins; iVertex++)
    {
        for (int iEta = 0; iEta < nEtaBins; iEta++)
        {
            for (int iET = 0; iET < nETBins; iET++)
            {
                ...
                //
                string binCut_temp = Form("vertex_z > %f && vertex_z <= %f && cluster_eta > %f && cluster_eta <= %f && cluster_ET > %f && cluster_ET <= %f && (et1>0.00268429*cluster_ET+0.876571) && (weta_cogx<0.0139207*cluster_ET+0.0479295)",

                                      vertexBins[iVertex], vertexBins[iVertex + 1],
                                      etaBins[iEta], etaBins[iEta + 1],
                                      etBins[iET], etBins[iET + 1]);
                if(sel=="ps"){
                  binCut_temp += " && (et1>0.00268429*cluster_ET+0.876571) && (weta_cogx<0.0139207*cluster_ET+0.0479295)";
                }

                TString binCut = binCut_temp; 
                ...
                */
                ...
                dataloader->PrepareTrainingAndTestTree("", "",
                                                       "nTrain_Signal=10000:nTrain_Background=0:SplitMode=Random:NormMode=NumEvents");
                ...
                factory->BookMethod(dataloader, TMVA::Types::kCuts, methodName,
                                    "!H:!V:VarProp=FSmart:PopSize=10:Cycles=1:CutRangeMin=-1:CutRangeMax=2");

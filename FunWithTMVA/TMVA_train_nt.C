...
void TMVA_train_nt(int ivtx, int ieta, int iet)
{
    ...
    TFile *outputFile = TFile::Open(Form("results/nt_TMVA_BinOptimization_v%d_eta%d_ET%d.root", ivtx, ieta, iet), "RECREATE");
    outputFile->cd();
    ...
    */
    ...
    const int nETBins = 4;
    double etBins[nETBins + 1] = {8, 10, 14, 20, 30};

    std::ofstream outFile(Form("results/nt_TMVA_BinOptimization_v%d_eta%d_ET%d.txt", ivtx, ieta, iet));
    if (!outFile)
    {
    ...
    for (int iVertex = 0; iVertex < nVertexBins; iVertex++)
    {
        for (int iEta = 0; iEta < nEtaBins; iEta++)
        {
            for (int iET = 0; iET < nETBins; iET++)
            {
                ...
                */
                dataloader->AddTree(backgroundTree10, "Signal", jet10weight, selection);
                dataloader->AddTree(backgroundTree15, "Signal", jet15weight, selection);
                dataloader->AddTree(backgroundTree20, "Signal", jet20weight, selection);
                dataloader->AddTree(backgroundTree30, "Signal", jet30weight, selection);
                dataloader->AddTree(signalTree5, "Background", photon5weight, selection);
                dataloader->AddTree(signalTree10, "Background", photon10weight, selection);
                dataloader->AddTree(signalTree20, "Background", photon20weight, selection);

                // Prepare the training and test trees.
                // The options here set the number of training events to all available events,
                // with a random split.
                dataloader->PrepareTrainingAndTestTree("", "",
                                                       "nTrain_Signal=0:nTrain_Background=10000:SplitMode=Random:NormMode=NumEvents");
                ...
                factory->BookMethod(dataloader, TMVA::Types::kCuts, methodName,
                                    "!H:!V:VarProp=FSmart:PopSize=300:Cycles=3:CutRangeMin=-1:CutRangeMax=2");
